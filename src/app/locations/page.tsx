...
export default async function LocationsPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-24">
      ...
      {/* Each pair of cards shares a subgrid so sections line up across columns */}
      <div className="flex flex-col gap-6">
        {chunk(locations, 2).map((pair, pi) => (
          <div
            key={pi}
                ...
                {loc.amenities?.length > 0 ? (
                  <div className="flex flex-wrap gap-1.5 px-8 pb-5">
                    {loc.amenities.map((a) => (
                      <span
                        key={a}
                        className="text-ghost text-[11px] uppercase tracking-[0.1em] bg-ink px-2.5 py-1"
                      >
                        {a}
                      </span>
                    ))}
