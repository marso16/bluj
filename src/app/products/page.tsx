...
export default async function ProductsPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-24">
      ...
      <h1 className="font-display font-black text-clean text-6xl md:text-8xl mb-20">Products</h1>
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-px bg-surface/50">
                ...
                <div className="col-span-2 md:col-span-1 bg-ink row-span-2">
                  ...
                  <div className="p-6">
                    ...
                    {first.description && (
                      <p className="text-ghost text-sm mt-2 leading-relaxed">{first.description}</p>
                    )}
                  </div>
                </div>
                ...
                {rest.map(p => (
                  <div key={p._id} className="bg-ink">
                    ...
                    <div className="p-4">
                      ...
                      {p.description && (
                        <p className="text-ghost text-xs mt-1 leading-relaxed line-clamp-2">{p.description}</p>
                      )}
                    </div>
                  </div>
                ))}
      ...
      {products.length === 0 && (
        <p className="text-ghost text-sm">No products found — add them in the Studio.</p>
      )}
    </div>
  )
}
